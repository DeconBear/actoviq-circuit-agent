...
async function writeLocalRenderFallbackArtifacts(
  ...
  if (!['netlistsvg-renderer', 'schemdraw-renderer', 'svg-layout-agent'].includes(stage.key)) {
    return record;
  }
  ...
  try {
      ...
      if (!(await fileExists(paths.netlistsvgNotesPath))) {
        await writeFile(
          ...
          [
            ...
            '- netlistsvg is the primary schematic renderer; schemdraw and agent SVG are secondary comparison paths.',
    ...
    } else if (stage.key === 'schemdraw-renderer') {
      const result = await runSchemdrawPipeline({
        designJsonPath: paths.designJsonPath,
        svgPath: paths.schemdrawPath,
      });
      if (result.ok) {
        notes.push('Rendered schemdraw schematic via local fallback pipeline.');
      }
      if (!(await fileExists(paths.schemdrawNotesPath))) {
        await writeFile(
          paths.schemdrawNotesPath,
          [
            '# Schemdraw Notes',
            '',
            '- This note was generated by local fallback rendering.',
            `- Source design JSON: ${paths.designJsonPath}`,
            `- Output SVG: ${paths.schemdrawPath}`,
            '',
          ].join('\n'),
          'utf8',
        );
      }
    } else if (stage.key === 'svg-layout-agent') {
      if (!(await fileExists(paths.sceneHintsPath))) {
        await writeJson(paths.sceneHintsPath, {
          title: 'Local fallback scene hints',
          placements: [],
          annotations: [
            {
              text: 'Generated by local fallback',
              position: [56, 56],
              font_size: 12,
            },
          ],
        });
        notes.push('Created default scene-hints.json for local SVG layout fallback.');
      }
      const result = await runAgentSvgPipeline({
        designJsonPath: paths.designJsonPath,
        svgPath: paths.agentSvgPath,
        scenePath: paths.sceneHintsPath,
        title: 'Fallback Agent Layout',
      });
      if (result.ok) {
        notes.push('Rendered agent-layout SVG via local fallback pipeline.');
      }
      if (!(await fileExists(paths.agentSvgNotesPath))) {
        await writeFile(
          paths.agentSvgNotesPath,
          [
            '# Agent Layout Notes',
            '',
            '- This note was generated by local fallback rendering.',
            `- Source design JSON: ${paths.designJsonPath}`,
            `- Scene hints: ${paths.sceneHintsPath}`,
            `- Output SVG: ${paths.agentSvgPath}`,
            '',
          ].join('\n'),
          'utf8',
        );
